...
// ── Config ─────────────────────────────────────────────────────────────────────
const API = process.env.NEXT_PUBLIC_DASHBOARD_API_URL ?? 'http://localhost:8003'
...
interface SummaryCards {
  ...
  ytd_from: string; ytd_to: string
  sply_from: string; sply_to: string
}

interface MonthPoint {
  month: number; month_label: string
  current_year: number; last_year: number
}

interface MLRPoint {
  month: number; month_label: string
  cumulative_mlr_pct: number
}

interface MonthlyTrends {
  cash_collection: MonthPoint[]
  claims_by_encounter: MonthPoint[]
  claims_by_submitted: MonthPoint[]
  pa_cost: MonthPoint[]
  cumulative_mlr_encounter: MLRPoint[]
  cumulative_mlr_submitted: MLRPoint[]
  current_year: number; last_year: number
}
...
interface TopClientRow { rank: number; group_name: string; amount_ytd: number }
interface TopProviderRow { rank: number; provider_id: string; provider_name: string; provider_band: string | null; amount_ytd: number }

interface CashBreakPoint {
  year: number; month: number; month_label: string
  total_cash: number
  salary_palliative: number; expense: number; commission: number
  salary_palliative_pct: number; expense_pct: number; commission_pct: number
  remaining: number; remaining_pct: number
}

// ── Helpers ────────────────────────────────────────────────────────────────────
const N = (n: number | null | undefined, dec = 0) =>
  n == null ? '—' : `₦${n.toLocaleString('en-NG', { minimumFractionDigits: dec, maximumFractionDigits: dec })}`
...
function mlrColor(pct: number) {
  if (pct <= 60) return 'text-emerald-600'
  if (pct <= 80) return 'text-amber-600'
  return 'text-rose-600'
}

function mlrBg(pct: number) {
...
function yoyArrow(cur: number, prev: number) {
  ...
  const up = diff >= 0
  return (
    <span className={`text-xs font-semibold ml-1 ${up ? 'text-rose-500' : 'text-emerald-500'}`}>
      {up ? '▲' : '▼'} {Math.abs(diff).toFixed(1)}% vs SPLY
    </span>
  )
}

// ── Tooltip formatter ──────────────────────────────────────────────────────────
function nairaFmt(value: number) { return Mn(value) }
  ...
  return (
    <div className="p-8 text-center">
      <div className="bg-rose-50 border border-rose-200 rounded-xl p-6 text-rose-700 max-w-md mx-auto">
        ...
        <p className="text-xs mt-3 text-slate-400">Is the dashboard API running? <span className="font-mono">{API}</span></p>
      </div>
    </div>
  )
}
...
function SummarySection({ d }: { d: SummaryCards }) {
  const CY = new Date(d.ytd_to).getFullYear()
  const PY = CY - 1

  const cards = [
    {
      label: `Cash Collected ${CY}`,
      val: d.cash_collected_ytd,
      prev: d.cash_collected_sply,
      color: 'bg-blue-50 border-blue-200',
      text: 'text-blue-700',
    },
    {
      label: `Claims Paid (Encounter) ${CY}`,
      val: d.claims_paid_encounter_ytd,
      prev: d.claims_paid_encounter_sply,
      color: 'bg-indigo-50 border-indigo-200',
      text: 'text-indigo-700',
    },
    {
      label: `Claims Paid (Submitted) ${CY}`,
      val: d.claims_paid_submitted_ytd,
      prev: d.claims_paid_submitted_sply,
      color: 'bg-violet-50 border-violet-200',
      text: 'text-violet-700',
    },
    {
      label: `Cash Collected ${PY}`,
      val: d.cash_collected_sply,
      prev: null,
      color: 'bg-slate-50 border-slate-200',
      text: 'text-slate-700',
    },
    {
      label: `Claims (Encounter) ${PY}`,
      val: d.claims_paid_encounter_sply,
      prev: null,
      color: 'bg-slate-50 border-slate-200',
      text: 'text-slate-700',
    },
    {
      label: `Claims (Submitted) ${PY}`,
      val: d.claims_paid_submitted_sply,
      prev: null,
      color: 'bg-slate-50 border-slate-200',
      text: 'text-slate-700',
    },
    {
      label: `MLR (Encounter) ${CY}`,
      val: null,
      pct: d.mlr_encounter_pct,
      prev: null,
      color: mlrBg(d.mlr_encounter_pct),
      text: mlrColor(d.mlr_encounter_pct),
    },
    {
      label: `MLR (Submitted) ${CY}`,
      val: null,
      pct: d.mlr_submitted_pct,
      prev: null,
      color: mlrBg(d.mlr_submitted_pct),
      text: mlrColor(d.mlr_submitted_pct),
    },
  ]
  ...
  return (
    <section>
      <SectionHead
        title="KPI Summary"
        subtitle={`YTD: ${d.ytd_from} → ${d.ytd_to}  ·  SPLY: ${d.sply_from} → ${d.sply_to}`}
      />
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {cards.map(c => (
          <div key={c.label} className={`rounded-xl border p-4 ${c.color}`}>
            ...
            {c.prev != null && c.val != null && (
              <div className="mt-1">{yoyArrow(c.val, c.prev)}</div>
            )}
            {c.val != null && (
              <p className="text-xs text-slate-400 mt-0.5">{N(c.val)}</p>
            )}
          </div>
        ))}
      </div>
...
function TrendsSection({ d }: { d: MonthlyTrends }) {
  ...
  const TABS: { id: TrendTab; label: string }[] = [
    { id: 'cash',    label: 'Cash' },
    { id: 'enc',     label: 'Claims (Enc)' },
    { id: 'sub',     label: 'Claims (Sub)' },
    { id: 'pa',      label: 'PA Cost' },
    { id: 'mlr_enc', label: 'MLR (Enc)' },
    { id: 'mlr_sub', label: 'MLR (Sub)' },
  ]

  const barSeries: Record<string, MonthPoint[]> = {
    cash: d.cash_collection,
    enc:  d.claims_by_encounter,
    sub:  d.claims_by_submitted,
    pa:   d.pa_cost,
  }

  const isBar = tab in barSeries
  const isMlr = tab === 'mlr_enc' || tab === 'mlr_sub'
  const mlrData = tab === 'mlr_enc' ? d.cumulative_mlr_encounter : d.cumulative_mlr_submitted
  ...
  return (
    <section>
      ...
      {isMlr && (
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={mlrData} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
            <XAxis dataKey="month_label" tick={{ fontSize: 11 }} />
...
// ── SECTION 3 — Contract Summary Table ────────────────────────────────────────
    ...
    <section>
      ...
      <div className="mb-3">
        <input
          ...
          className="border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#137fec] w-64"
        />
      </div>
      <div className="overflow-x-auto rounded-xl border border-slate-200 shadow-sm">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {filtered.map(r => {
              const pct = r.cash_vs_debit_pct
              const pctColor = pct == null ? '' : pct >= 100 ? 'text-emerald-600 font-bold' : pct >= 75 ? 'text-amber-600' : 'text-rose-600'
              return (
                <tr key={r.group_name} className="hover:bg-slate-50 transition-colors">
                  <td className="px-4 py-3 font-medium text-slate-800 max-w-[200px] truncate">{r.group_name.trim()}</td>
                  <td className="px-4 py-3 text-xs text-slate-500 whitespace-nowrap">
                    {r.previous_contract_start
                      ? `${r.previous_contract_start} → ${r.previous_contract_end}`
                  ...
                  <td className={`px-4 py-3 tabular-nums whitespace-nowrap ${pctColor}`}>{Pct(pct)}</td>
                </tr>
              )
            })}
          </tbody>
        ...
        {filtered.length === 0 && (
          <p className="text-center py-8 text-slate-400 text-sm">No results</p>
        )}
      </div>
      <p className="text-xs text-slate-400 mt-2">{filtered.length} of {rows.length} companies shown</p>
    </section>
  )
}

// ── SECTION 4 — Top 20 Tables ──────────────────────────────────────────────────
type Top20Tab = 'pa_clients' | 'pa_providers' | 'enc_clients' | 'sub_clients'

function Top20Section({
  paClients, paProviders, encClients, subClients,
}: {
  paClients: TopClientRow[]; paProviders: TopProviderRow[]
  encClients: TopClientRow[]; subClients: TopClientRow[]
}) {
  ...
  const TABS: { id: Top20Tab; label: string }[] = [
    ...
    { id: 'enc_clients',   label: 'Top Clients (Claims Enc)' },
    { id: 'sub_clients',   label: 'Top Clients (Claims Sub)' },
  ]
  ...
  function BandBadge({ band }: { band: string | null }) {
    ...
    return (
      <span className={`inline-block text-xs font-bold px-2 py-0.5 rounded-full ${colors[k] ?? 'bg-slate-100 text-slate-600'}`}>
        {band}
      </span>
    )
  }
      ...
      <div className="overflow-x-auto rounded-xl border border-slate-200 shadow-sm">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            ...
            {tab === 'sub_clients' && subClients.map(r => (
              <tr key={r.rank} className="hover:bg-slate-50">
                <td className="px-4 py-3 text-slate-400 font-mono text-xs">{r.rank}</td>
                <td className="px-4 py-3 font-medium text-slate-800">{r.group_name}</td>
                <td className="px-4 py-3 text-right tabular-nums font-semibold text-[#137fec]">{Mn(r.amount_ytd)}</td>
...
// ── SECTION 5 — Cash Breakdown Stacked Bar ────────────────────────────────────
function CashBreakSection({ rows }: { rows: CashBreakPoint[] }) {
  ...
  const filtered = rows.filter(r => !year || r.year === year)
    ...
    <section>
      ...
      <ResponsiveContainer width="100%" height={320}>
        <BarChart data={chartData} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
          ...
          <YAxis
            tickFormatter={v => mode === 'pct' ? `${v}%` : Mn(v)}
            tick={{ fontSize: 11 }} width={mode === 'pct' ? 40 : 72}
          />
          <Tooltip
            formatter={(v: number, name: string) =>
              mode === 'pct' ? [`${v.toFixed(1)}%`, name] : [Mn(v), name]
            }
          />
          <Legend wrapperStyle={{ fontSize: 12 }} />
      ...
      <div className="overflow-x-auto rounded-xl border border-slate-200 shadow-sm mt-4">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {filtered.map(r => {
              const overhead = r.salary_palliative_pct + r.expense_pct + r.commission_pct
              return (
                <tr key={`${r.year}-${r.month}`} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 tabular-nums font-bold whitespace-nowrap">
                    <span className={overhead > 70 ? 'text-rose-600' : overhead > 50 ? 'text-amber-600' : 'text-slate-700'}>
                      {overhead.toFixed(1)}%
                    </span>
                  </td>
                </tr>
...
export default function ExecutiveDashboard() {
  ...
  const [paClients,     setPaClients]     = useState<TopClientRow[]>([])
  const [paProviders,   setPaProviders]   = useState<TopProviderRow[]>([])
  const [encClients,    setEncClients]    = useState<TopClientRow[]>([])
  const [subClients,    setSubClients]    = useState<TopClientRow[]>([])
  const [cashBreak,     setCashBreak]     = useState<CashBreakPoint[]>([])
  ...
  async function load() {
    ...
    try {
      const [s, t, c, pc, pp, ec, sc, cb] = await Promise.all([
        fetch(`${API}/dashboard/summary-cards`).then(r => r.json()),
        fetch(`${API}/dashboard/monthly-trends`).then(r => r.json()),
        fetch(`${API}/dashboard/contract-summary`).then(r => r.json()),
        fetch(`${API}/dashboard/top-clients/pa`).then(r => r.json()),
        fetch(`${API}/dashboard/top-providers/pa`).then(r => r.json()),
        fetch(`${API}/dashboard/top-clients/claims-encounter`).then(r => r.json()),
        fetch(`${API}/dashboard/top-clients/claims-submitted`).then(r => r.json()),
        fetch(`${API}/dashboard/cash-breakdown`).then(r => r.json()),
      ])
      if (s.detail) throw new Error(s.detail)
      setSummary(s); setTrends(t); setContracts(c)
      setPaClients(pc); setPaProviders(pp); setEncClients(ec); setSubClients(sc)
      setCashBreak(cb)
    } catch (e: unknown) {
  ...
  return (
    <>
      ...
      <header className="h-16 border-b border-slate-200 bg-white flex items-center justify-between pl-14 pr-4 md:px-8 sticky top-0 z-10">
        ...
        <button
          onClick={load}
          className="text-xs text-slate-500 hover:text-[#137fec] flex items-center gap-1.5 transition-colors"
        >
          ↺ Refresh
        </button>
      </header>
      ...
      <div className="p-4 md:p-8 space-y-10">
        {summary && <SummarySection d={summary} />}
        {trends   && <TrendsSection d={trends} />}
        {contracts.length > 0 && <ContractSection rows={contracts} />}
        {paClients.length > 0 && (
          <Top20Section
            paClients={paClients} paProviders={paProviders}
            encClients={encClients} subClients={subClients}
          />
        )}
