        ...
        <div className="bg-white border border-amber-200 rounded-lg overflow-hidden mb-6">
          <div className="px-6 py-4 border-b border-amber-200 bg-amber-50 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-amber-700 mt-0.5">Review the snapshot, then approve for bidding, deny with a comment, or send directly to an aggregator.</p>
            </div>
            ...
            <tbody>
              {pendingReview.map((order, idx) => {
                ...
                return (
                  ...
                  <tr className="border-b border-slate-200"><td colSpan={6} className="px-4 pb-4 space-y-3">
                    <ReviewFlagsPanel flags={order.reviewFlags} />
                    {order.status === 'rejected' && order.denialComment && <p className="text-sm text-rose-700 whitespace-pre-wrap">Denial comment: {order.denialComment}</p>}
                    <ReviewActions key={`${order.id}-${order.status}`} order={order} onComplete={load} />
                  </td></tr>
                  </Fragment>
                )
              })}
